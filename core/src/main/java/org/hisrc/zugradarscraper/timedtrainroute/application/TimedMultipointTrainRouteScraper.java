...
public class TimedMultipointTrainRouteScraper {
	...
	public static class Configuration {
		...
		@Argument(metaVar = "OUTPUT", required = false, index = 0, usage = "Output GeoJSON file with timed multipoint train routes, timedMultipointTrainRoutes.geojson by default")
		private File timedTrainRoutesFile = new File("timedMultipointTrainRoutes.geojson");
	...
	private final Logger LOGGER = LoggerFactory.getLogger(TimedMultipointTrainRouteScraper.class);
	...
	public static void main(String[] args) throws IOException {
		...
		try {
			...
			if (configuration.isHelp()) {
				...
				new TimedMultipointTrainRouteScraper().scrapeTrainRoutes(configuration.getTrainIdsFile(),
	...
	private static void printUsage(CmdLineParser parser) {
		System.out.println("Usage: java -cp zugradarscraper.jar " + TimedMultipointTrainRouteScraper.class.getName()
				+ "[options...] [OUTPUT]");
	...
	public void scrapeTimedTrainRoutes(final List<TrainId> trainIds, OutputStream trainRoutesOutputStream) {
		final List<TimedTrainRouteMultiPointFeature> trainRoutes = new ArrayList<>(trainIds.size());

		for (TrainId trainId : trainIds) {
			try {
				final TimedTrainRoute trainRoute = trainRouteRetriever.retrieve(trainId);
				if (trainRoute != null) {
					final TimedTrainRouteMultiPointFeature feature = trainRoute.interpolateTrainRouteMultiPointFeature(10000);
		...
		final TimedTrainRouteMultiPointFeatureCollection trainRouteCollection = new TimedTrainRouteMultiPointFeatureCollection(
				trainRoutes);

		final ObjectMapper objectMapper = new ObjectMapper();
		objectMapper.registerModule(new JavaTimeModule());
		try {
			objectMapper.writerFor(TimedTrainRouteMultiPointFeatureCollection.class).withDefaultPrettyPrinter()
